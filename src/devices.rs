...
impl SharedDevices {
    ...
    pub fn get_buid(&self) -> Result<String, PlistError> {
        ...
        if !path.exists() {
            ...
            let mut new_plist = Plist::new_dict();
            let new_udid = uuid::Uuid::new_v4();
            new_plist.dict_insert_item(
                "SystemBUID",
                Plist::new_string(new_udid.to_string().as_str()),
            )?;
            let new_plist = new_plist.to_string();
            if let Ok(mut f) = std::fs::File::create_new(&path) {
                if let Err(e) = f.write_all(new_plist.as_bytes()) {
                    error!("Unable to save SystemConfiguration.plist: {e:?}");
